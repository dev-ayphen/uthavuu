...
describe('push provider selection', () => {
  ...
  it.each([
    ...
    ['only a service account', { FCM_SERVICE_ACCOUNT_JSON: JSON.stringify(SERVICE_ACCOUNT) }],
  ...
  it('prefers real credentials over the fallback even in production', () => {
    const provider = createPushProvider({ ...REAL_CREDENTIALS, NODE_ENV: 'production' });
...
describe('production hard-block', () => {
  ...
  it('refuses to start in production with only half the credentials', () => {
    expect(() =>
      createPushProvider({ NODE_ENV: 'production', FCM_PROJECT_ID: 'uthavu-test' }),
...
describe('service account parsing', () => {
  ...
  it('accepts the same JSON base64-encoded', () => {
    ...
    expect(parseServiceAccount(encoded).clientEmail).toBe(SERVICE_ACCOUNT.client_email);
  });
  ...
  it('rejects a service account from a different project than FCM_PROJECT_ID', async () => {
    ...
    await expect(provider.sendToTokens(['tok'], { title: 't', body: 'b' })).rejects.toThrow(
      /does not match the project_id/,
    );
  });
});
...
describe('dead-token classification', () => {
  ...
  it('never treats invalid-argument as a dead token', () => {
    const dead = collectDeadTokens(
      tokens.map(() => ({ success: false, error: { code: 'messaging/invalid-argument' } })),
      tokens,
    );
    expect(dead).toEqual([]);
  });
